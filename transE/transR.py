...
class TransEModel(object):
	...
	def __init__(self, config):
		...
		with tf.name_scope("embedding"):
			...
			self.rel_matrix = tf.get_variable(name = "rel_matrix", shape = [relation_total, size * size], initializer = tf.contrib.layers.xavier_initializer(uniform = False))
			...
			matrix = tf.nn.embedding_lookup(self.rel_embeddings, self.neg_r)

			# pos_h_e = tf.nn.l2_normalize(tf.nn.embedding_lookup(self.ent_embeddings, self.pos_h), 1)
			# pos_t_e = tf.nn.l2_normalize(tf.nn.embedding_lookup(self.ent_embeddings, self.pos_t), 1)
			# pos_r_e = tf.nn.l2_normalize(tf.nn.embedding_lookup(self.rel_embeddings, self.pos_r), 1)
			# neg_h_e = tf.nn.l2_normalize(tf.nn.embedding_lookup(self.ent_embeddings, self.neg_h), 1)
			# neg_t_e = tf.nn.l2_normalize(tf.nn.embedding_lookup(self.ent_embeddings, self.neg_t), 1)
			# neg_r_e = tf.nn.l2_normalize(tf.nn.embedding_lookup(self.rel_embeddings, self.neg_r), 1)
