	 		...
	 		for times in range(config.trainTimes):
				...
				if (times % 100 == 0):
					...
					for i in enb:
						...
						f.write("\n")
					f.close()
					f = open("matrix.bern", "w")
					mat = np.resize(trainModel.rel_matrix.eval(),(config.relation, config.hidden_sizeR, config.hidden_sizeE))
					for i in range(config.relation):
						for j in range(config.hidden_sizeE):
							for k in range(config.hidden_sizeR):
								f.write("%f\t"%mat[i][k][j])
					f.close()
